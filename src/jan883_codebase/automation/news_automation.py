...
# Scrape the websites
news = scrape_website(['https://arstechnica.com/gadgets/', 'https://arstechnica.com', 'https://arstechnica.com/information-technology/'], display_text=False)

# Zip the titles and news content together
news_stream = zip(title, news)

# Send the image with a caption
response = telegram_send_image("images/ARS.png", caption=f"Arstechnica {human_readable_time}")

# Iterate over the zipped titles and news content
for t, n in news_stream:
    # Summarize the content using ask_ollama
    my_message = ask_ollama(f"Summarize this website content for me in bullet points: {n}", format="plain text")
