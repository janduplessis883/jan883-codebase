...
from data_preprocessing.eda import scale_X_train_X_test, define_X_y, sample_df, individual_t_test_classification
...
def test_scale_X_train_X_test_standard():
    ...
    # Call the function
    scaled_X_train, scaled_X_test = scale_X_train_X_test(X_train, X_test, scaler="standard")

    # Check that the data is scaled correctly using StandardScaler
    scaler = StandardScaler().fit(X_train)
    expected_scaled_X_train = pd.DataFrame(scaler.transform(X_train), columns=X_train.columns)
    expected_scaled_X_test = pd.DataFrame(scaler.transform(X_test), columns=X_test.columns)
...
def test_scale_X_train_X_test_minmax():
    ...
    # Call the function
    scaled_X_train, scaled_X_test = scale_X_train_X_test(X_train, X_test, scaler="minmax")
...
def test_scale_X_train_X_test_robust():
    ...
    # Call the function
    scaled_X_train, scaled_X_test = scale_X_train_X_test(X_train, X_test, scaler="robust")
...
def test_invalid_scaler_type():
    ...
    with pytest.raises(ValueError, match='Invalid scaler type. Choose "standard", "minmax", or "robust".'):
...
def test_save_scaler():
    ...
    saved_files = [f for f in os.listdir('.') if f.startswith('scaler_') and f.endswith('.pkl')]
...
def test_sample_df_more_samples_than_rows(capsys):
    ...
    assert "The number of samples is greater than the number of rows in the dataframe." in captured.out
...
def test_individual_t_test_classification_basic():
    ...
    # Call the function
    result = individual_t_test_classification(df, 'target', 0, 1, ['feature1', 'feature2'])
...
def test_individual_t_test_classification_with_alpha():
    ...
    result = individual_t_test_classification(df, 'target', 0, 1, ['feature1', 'feature2'], alpha_val=0.1)

    # Check if the significance is correctly identified
    assert result[result['feature'] == 'feature1']['significance'].values[0] == 'Significant'
    assert result[result['feature'] == 'feature2']['significance'].values[0] == 'Insignificant'
...
def test_individual_t_test_classification_sampling():
    ...
    result = individual_t_test_classification(df, 'target', 0, 1, ['feature1', 'feature2'], sample_frac=0.5, random_state=42)
